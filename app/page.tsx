  ...
  return <main className="app-shell" aria-busy={isWorking}>
    ...
    <section className="workspace">
      ...
      <aside className="sidebar controls-sidebar">
        ...
        <section className="slicer-handoff" aria-label="Export slicer project"><div className="label-with-help"><div><strong>Export slicer project</strong><span>{plates.length > 1 ? "One standards-compliant 3MF per plate, packaged in a ZIP." : "A standards-compliant 3MF for the active plate."}</span></div><HelpTip label="local slicer export">Every mesh is checked for finite vertices, non-degenerate triangles, and closed manifold edges before download. Multi-plate work is exported as separate standard 3MF files because multi-plate project metadata is slicer-specific. Nothing is uploaded.</HelpTip></div><select aria-label="Target slicer" value={preferredSlicer} onChange={(event) => setPreferredSlicer(event.target.value as SlicerTarget)}><option value="orca">OrcaSlicer</option><option value="bambu">Bambu Studio</option><option value="prusa">PrusaSlicer</option></select><button className="button primary slicer-download" disabled={!allPlacements.length || isWorking} onClick={download3mf}>{plates.length > 1 ? "Export verified plate ZIP" : "Export verified 3MF"}</button><p>Open each exported plate in your slicer. No desktop app is launched.</p></section>
