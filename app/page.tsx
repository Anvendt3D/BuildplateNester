...
import { create3mf, createPlateArchive } from "./three-mf";
...
export default function Home() {
  ...
  function download3mf() {
    try {
      ...
      const blob = multiPlate ? createPlateArchive(parts, allPlacements, exportPlates) : create3mf(parts, allPlacements);
      const url = URL.createObjectURL(blob), download = document.createElement("a"); download.href = url; download.download = multiPlate ? "printnest-plates.zip" : "printnest-plate.3mf"; download.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 20_000);
      setMessage(multiPlate ? "Downloaded one standards-compliant 3MF per plate in a local ZIP." : "Downloaded the active 3MF project."); return true;
  ...
  return <main className="app-shell" aria-busy={isWorking}>
    ...
    <section className="workspace">
      ...
      <aside className="sidebar controls-sidebar">
        ...
        <details className="workflow-help"><summary>How to use PrintNest <span>＋</span></summary><ol><li>Import STEP or STL files. They stage on the active plate; overflow remains unassigned.</li><li>Orient models, select the printer, and manage plates from the tabs above the build area.</li><li>Lock completed plates or exact placements before further nesting.</li><li>Use <b>Nest current</b>, <b>Nest all</b>, production sets, or consolidation as needed.</li><li>Each plate is exported as portable Core 3MF; multi-plate jobs are packaged as a ZIP.</li></ol><p>Everything stays local. Every operation enforces the fixed 2 mm bed-edge safety boundary.</p></details>
        ...
        <section className="slicer-handoff" aria-label="Export 3MF"><div className="label-with-help"><div><strong>Export 3MF</strong><span>{plates.length > 1 ? "A ZIP with one standard 3MF per plate." : "A standard 3MF for the active plate."}</span></div><HelpTip label="3MF export">The export follows the public 3MF Core specification. It contains only model geometry and placement—no slicer profile, vendor extension, account data, or upload.</HelpTip></div><button className="button primary slicer-download" disabled={!allPlacements.length || isWorking} onClick={download3mf}>{plates.length > 1 ? "Export 3MF plate ZIP" : "Export 3MF"}</button><p>Open the exported 3MF in any slicer that supports the Core 3MF format.</p></section>
