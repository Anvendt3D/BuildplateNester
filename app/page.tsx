...
import { ChangeEvent, DragEvent, lazy, PointerEvent, Suspense, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
import { displayFootprint, Footprint, footprintArea, footprintBounds, footprintFromRing, footprintPath, footprintsOverlap, silhouetteFromMeshes, transformFootprint, translateFootprint } from "./footprint";
...
const OrientationViewer = lazy(() => import("./orientation-viewer"));
const PlateModelPreview = lazy(() => import("./plate-model-preview"));

type Part = {
  id: string; name: string; quantity: number; height: number; footprint: Footprint; displayFootprint?: Footprint;
...
function renderedFootprint(p: Placement, part: Part) { return transformFootprint(part.displayFootprint ?? p.footprint, p.rotation, p.x, p.y); }
...
function importStepFile(text: string): Promise<ModelMesh & { diagnostics?: { ok?: boolean; openEdges?: number; nonManifoldEdges?: number } }> {
...
function updateGeometry(part: Part, orientation: QuaternionTuple): Part {
  ...
  const footprint = silhouetteFromMeshes(workingMeshes, orientation);
  return { ...part, orientation, footprint, displayFootprint: displayFootprint(footprint), height: maxZ - minZ };
}
...
export default function Home() {
  ...
  const [printerId, setPrinterId] = useState("bambu-p1"), [clearance, setClearance] = useState(2), [clearanceDraft, setClearanceDraft] = useState("2.0"), [autoRotate, setAutoRotate] = useState(true), [rotationEffort, setRotationEffort] = useState<RotationEffort>("balanced"), [nestingStart, setNestingStart] = useState<NestingStart>("corner"), [outlinePrecision, setOutlinePrecision] = useState<OutlinePrecision>("standard");
  ...
  const nestActionLabel = primaryNestAction === "current" ? "Nest on active plate" : primaryNestAction === "all" ? "Nest across existing plates" : primaryNestAction === "add" ? "Nest and add plates" : plates.length > 1 || unplacedCount > 0 ? "Nest across plates" : "Nest on active plate";
  const nestScopeNote = primaryNestAction === "current" ? "Keep every other plate unchanged." : primaryNestAction === "all" ? "Redistribute unlocked parts across the plates you already have." : primaryNestAction === "add" ? "Use this plate first, then add plates only for overflow." : "PrintNest will use the active plate first and add space only when needed.";
  const searchQualityNote = searchPreset === "quick" ? "Fast first pass: checks the main rotations." : searchPreset === "best" ? "Most thorough: explores fine rotations and tighter interlocks." : "Recommended: balances a compact result with practical search time.";
  const partReadiness = (part: Part) => { const b = bounds(part.footprint), width = b.maxX - b.minX, depth = b.maxY - b.minY; if (part.height > bedHeight) return { tone: "blocked", text: `Too tall for this printer (${part.height.toFixed(1)} / ${bedHeight} mm)` }; if (width > bedWidth - EDGE_MARGIN * 2 || depth > bedDepth - EDGE_MARGIN * 2) return { tone: "blocked", text: "Footprint exceeds the usable build area" }; return { tone: "ready", text: part.meshes.length ? "Imported pose · ready to nest" : "Ready to nest" }; };
    ...
    const jobs = attempts.map((attempt, index) => new Promise<NestBatchResult>((resolve, reject) => {
      ...
      worker.onmessage = (event: MessageEvent<{ type: "progress"; progress: NestProgress; jobId?: string } | { type: "result"; result: NestBatchResult; jobId?: string } | { type: "error"; message: string; jobId?: string }>) => {
        ...
        if (event.data.type === "progress") {
          ...
          const progress = [...progressByPass.values()], processed = Math.max(0, ...progress.map((item) => item.processed)), total = eligible.reduce((sum, part) => sum + (part.copies?.length ?? part.quantity), 0);
  ...
  function applyOrientation() {
    ...
    commitOrientation(orientingId, updateGeometry(orientingPart, orientingPart.orientation), `Applied the new orientation to ${orientingPart.name}.`); closeOrientation();
  }
  ...
  async function autoOrientCurrent() {
    ...
    try {
      ...
      setOrientationDraft(updateGeometry(source, result.orientation));
  ...
  async function fillPlate(partId: string, requestedMode: typeof fillMode = fillMode) {
    const target = parts.find((part) => part.id === partId); if (!target) return;
    const mode = requestedMode;
    if (activePlate?.locked && mode !== "existing" && mode !== "batch") { setMessage(`${activePlate.name} is locked. Choose another plate or unlock it.`); return; }
    ...
    try {
      ...
      if (mode === "remaining") {
        ...
        while (available.length && !localTaskCancelRef.current) {
          ...
          const batch = await runInWorker([{ ...target, quantity: wave.length, copies: wave, minQuantity: 0 }], rotationEffort, working, undefined, { singlePass: true });
      ...
      const fixed = mode === "remaining" ? placements : placements.filter((placement) => placement.locked);
      const fixedIds = new Set(fixed.map((placement) => placement.id));
      const requested = parts.map((part) => {
        ...
        const copies = Array.from({ length: limit }, (_, index) => index + 1).filter((copy) => {
          ...
          return mode === "repack" ? currentIds.has(id) || part.id === partId : part.id === partId && !placedGlobalIds.has(id);
        });
      ...
      setMessage(batch.cancelled ? `Stopped filling and kept the best completed result.` : mode === "remaining" ? `Filled unused space on ${activePlate?.name} without moving existing parts.` : `Filled ${activePlate?.name} with solver-nested instances of ${target.name}.`);
  ...
  async function distributeAcrossPlates(targetParts: Part[], allowCreate: boolean, consolidate: boolean) {
    ...
    try {
      ...
      while ([...pool.values()].some((copies) => copies.length)) {
        ...
        if (cancelled || next.length === fixed.length) break;
      }
      if (consolidate) {
  ...
  function lockAllPlates() { rememberLayout(); setPlates((current) => current.map((plate) => ({ ...plate, locked: true }))); setMessage("All plates locked."); }
  function unlockAll() { rememberLayout(); setPlates((current) => current.map((plate) => ({ ...plate, locked: false }))); setAllPlacements((current) => current.map((placement) => ({ ...placement, locked: false }))); setMessage("All plates and placed parts unlocked."); }
  ...
  function layFaceDown(normal: [number, number, number]) {
    if (!orientingPart) return;
    const delta = quaternionFromUnitVectors(normalize3(vec3(...normal)), vec3(0, 0, -1));
    orientPart(orientingPart.id, multiplyQuaternion(delta, orientingPart.orientation), `Placed the clicked face of ${orientingPart.name} on the build plate.`);
  }
  ...
  function onPlatePointerMove(event: PointerEvent<SVGSVGElement>) {
    ...
    // Keep pointer movement to a cheap position update. Exact polygon
    // collisions are confirmed once the pointer is released; doing a complete
    // all-part collision pass for every pixel made dense plates feel sticky.
    setPlacements((current) => current.map((p) => p.id === dragging.id ? { ...p, x: dragging.originX + dx, y: dragging.originY + dy } : p));
  }
  function finishPlateDrag() {
    if (!dragging) return;
    setPlacements((current) => markCollisions(current, bedWidth, bedDepth, clearance)); setDragging(null);
  }
  function download3mf() {
    try {
      const exportPlates = plates.filter((plate) => allPlacements.some((placement) => placement.plateId === plate.id));
      const multiPlate = exportPlates.length > 1;
      const blob = multiPlate ? createMultiPlate3mf(parts, allPlacements, exportPlates, bedDepth) : create3mf(parts, allPlacements, bedDepth);
      const url = URL.createObjectURL(blob), download = document.createElement("a"); download.href = url; download.download = multiPlate ? "printnest-project.3mf" : "printnest-plate.3mf"; download.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 20_000);
      setMessage(multiPlate ? `Downloaded one ${exportPlates.length}-plate Bambu Studio / OrcaSlicer project 3MF.` : "Downloaded the portable Core 3MF build."); return true;
  ...
  return <main className="app-shell" aria-busy={isWorking}>
    ...
    <section className="workspace">
      <aside className="sidebar left-sidebar">
        ...
        <div className="part-list">{parts.map((part) => { const b = bounds(part.footprint), affected = showUnplaced && unplacedByPart.some((item) => item.part.id === part.id), placedTotal = allPlacements.filter((placement) => placement.partId === part.id).length, unassigned = Math.max(0, part.quantity - placedTotal), readiness = partReadiness(part); return <article className={`part-card ${selectedPartId === part.id ? "selected" : ""} ${affected ? "has-unplaced" : ""}`} key={part.id} onClick={() => { setSelectedPartId(part.id); setSelectedPlacementId(null); setSelectedPlacementIds([]); }}>
          ...
          <div className="part-placement-summary"><span>{part.quantity} requested · {placedTotal} placed · {unassigned} remaining</span>{unassigned > 0 && <button type="button" className="part-drag-handle" draggable onDragStart={(event) => { event.stopPropagation(); dragPayloadRef.current = { type: "part", partId: part.id }; }} onClick={(event) => event.stopPropagation()} aria-label={`Drag an unassigned ${part.name} to a plate`} title="Drag an unassigned copy to a plate">⠿</button>}</div>
          <p className={`part-readiness ${readiness.tone}`}>{readiness.text}</p>
          <div className="part-actions">{part.meshes.length > 0 && <button aria-label={`Review orientation for ${part.name}`} title="Review the imported pose or choose a face to place down" disabled={isWorking} onClick={() => openOrientation(part.id)}>Review orientation</button>}<details className="repeat-pattern"><summary aria-label={`Fill bed options for ${part.name}`}>Fill bed</summary><p>Uses the nesting solver to maximize instances on this plate. It may rotate and interlock copies.</p><button className="fill-action" aria-label={`Fill bed with instances of ${part.name}`} disabled={isWorking} onClick={() => fillPlate(part.id, "repack")}>Fill bed with instances</button></details><button aria-label={`Remove ${part.name}`} title="Remove this part and all its instances from every plate" disabled={isWorking} onClick={() => { rememberLayout(); setParts((current) => current.filter((p) => p.id !== part.id)); setAllPlacements((current) => current.filter((p) => p.partId !== part.id)); if (orientingId === part.id) closeOrientation(); if (selectedPartId === part.id) setSelectedPartId(null); }}>Remove</button></div>
      ...
      <section className="plate-stage">
        <nav className="plate-tabs" aria-label="Project plates">{plates.map((plate, index) => { const platePlacements = allPlacements.filter((placement) => placement.plateId === plate.id), plateUtilization = platePlacements.reduce((sum, placement) => sum + footprintArea(placement.footprint), 0) / Math.max(1, bedWidth * bedDepth) * 100; return <button key={plate.id} className={plate.id === activePlateId ? "active" : ""} onDragOver={(event) => { if (!plate.locked) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); dropOnPlate(plate.id); }} onClick={() => { setActivePlateId(plate.id); setSelectedPlacementId(null); setSelectedPlacementIds([]); setLayoutOptions([]); }}><svg viewBox={`0 0 ${bedWidth} ${bedDepth}`} preserveAspectRatio="none" aria-hidden="true"><rect x="0" y="0" width={bedWidth} height={bedDepth} />{platePlacements.map((placement) => <path key={placement.id} d={footprintPath(translated(placement))} />)}</svg><span><b>{plate.locked ? "🔒 " : ""}{plate.name}</b><small>{platePlacements.length} parts · {plateUtilization.toFixed(0)}%</small></span><em>{index + 1}</em></button>; })}<button className="add-plate-tab" aria-label="Add plate" onClick={() => addPlate()}>＋</button></nav>
        {plates.length > 1 && <section className="multi-plate-overview" aria-label="Side-by-side plate overview"><div><strong>All plates</strong><span>Click a plate to edit it</span></div><div className="multi-plate-overview-grid">{plates.map((plate, index) => { const platePlacements = allPlacements.filter((placement) => placement.plateId === plate.id); return <button key={plate.id} className={plate.id === activePlateId ? "active" : ""} onClick={() => { setActivePlateId(plate.id); setSelectedPlacementId(null); setSelectedPlacementIds([]); }}><svg viewBox={`0 0 ${bedWidth} ${bedDepth}`} preserveAspectRatio="none" aria-label={`${plate.name} overview`}><rect x="0" y="0" width={bedWidth} height={bedDepth} />{platePlacements.map((placement) => <path key={placement.id} d={footprintPath(translated(placement))} />)}</svg><span><b>{index + 1}. {plate.name}</b><small>{platePlacements.length} parts</small></span></button>; })}</div></section>}
        <div className="plate-admin"><label><span>Active plate</span><input aria-label="Active plate name" value={activePlate?.name ?? ""} disabled={isWorking} onChange={(event) => renameActivePlate(event.target.value)} /></label><p className="plate-overview">{plates.length} plate{plates.length === 1 ? "" : "s"} · {plates.filter((plate) => plate.locked).length} locked</p><div className="plate-actions"><button aria-label="Add plate" onClick={() => addPlate()}>Add plate</button><button onClick={togglePlateLock}>{activePlate?.locked ? "Unlock plate" : "Lock plate"}</button><button onClick={unlockAll}>Unlock plates + parts</button><button onClick={lockAllPlates}>Lock all plates</button><button onClick={duplicateActivePlate}>Duplicate</button><button onClick={clearActivePlate}>Clear</button><button onClick={deleteActivePlate} disabled={plates.length === 1}>Delete</button></div></div>
        ...
        {nestingMode && <div className="live-nesting" aria-live="polite"><i />Live preview · {placements.length} placed · {nestProgress.candidateChecks.toLocaleString()} positions tested</div>}
        <div className={`plate-wrap ${plates.length > 1 ? "multi-plate-source" : ""}`}><div className="plate-frame" style={{ aspectRatio: `${bedWidth}/${bedDepth}` }}><div className="dimension top"><span>{bedWidth} mm</span></div><div className="dimension side"><span>{bedDepth} mm</span></div><Suspense fallback={null}><PlateModelPreview width={bedWidth} depth={bedDepth} parts={parts} placements={placements} /></Suspense>
          <svg className={`build-plate ${activePlate?.locked ? "is-locked" : ""}`} viewBox={`0 0 ${bedWidth} ${bedDepth}`} style={{ aspectRatio: `${bedWidth}/${bedDepth}` }} onPointerMove={onPlatePointerMove} onPointerUp={finishPlateDrag} onPointerLeave={finishPlateDrag} aria-label="Top-down build plate nesting preview">
            <defs><pattern id="minorGrid" width={GRID} height={GRID} patternUnits="userSpaceOnUse"><path d={`M ${GRID} 0 L 0 0 0 ${GRID}`} fill="none" stroke="#2e2e38" strokeWidth="0.45" /></pattern><pattern id="majorGrid" width={GRID * 5} height={GRID * 5} patternUnits="userSpaceOnUse"><rect width={GRID * 5} height={GRID * 5} fill="url(#minorGrid)" /><path d={`M ${GRID * 5} 0 L 0 0 0 ${GRID * 5}`} fill="none" stroke="#3a3a47" strokeWidth="0.75" /></pattern></defs><rect width={bedWidth} height={bedDepth} fill="url(#majorGrid)" />
            ...
            {placements.map((placement) => { const part = parts.find((p) => p.id === placement.partId)!, footprint = renderedFootprint(placement, part), b = bounds(footprint), selected = selectedPlacementIds.includes(placement.id) || selectedPlacementId === placement.id || (!selectedPlacementId && !selectedPlacementIds.length && selectedPartId === placement.partId); return <g key={placement.id} className={`plate-part ${placement.nested ? "is-nested" : "is-staged"} ${placement.locked ? "is-locked" : ""} ${selected ? "is-selected" : ""} ${nestingMode ? "is-preview" : ""}`} onPointerDown={(event) => { const nextIds = event.shiftKey ? (selectedPlacementIds.includes(placement.id) ? selectedPlacementIds.filter((id) => id !== placement.id) : [...selectedPlacementIds, placement.id]) : selectedPlacementIds.includes(placement.id) ? selectedPlacementIds : [placement.id]; setSelectedPlacementIds(nextIds); setSelectedPartId(placement.partId); setSelectedPlacementId(nextIds[0] ?? placement.id); if (event.shiftKey || placement.locked || activePlate?.locked) return; rememberLayout(); event.currentTarget.setPointerCapture(event.pointerId); setDragging({ id: placement.id, startX: event.clientX, startY: event.clientY, originX: placement.x, originY: placement.y }); }}><path d={footprintPath(footprint)} fill="transparent" fillRule="evenodd" /><text x={(b.minX + b.maxX) / 2} y={(b.minY + b.maxY) / 2} textAnchor="middle" dominantBaseline="middle" fontSize={Math.max(4, 11 / scale)}>{placement.locked ? "🔒" : placement.copy}</text></g>; })}
        ...
        </div>{plates.length === 1 && <aside className="plate-control-rail single-plate-control-rail" aria-label="Active plate controls"><input aria-label="Active plate name" value={activePlate?.name ?? ""} disabled={isWorking} onChange={(event) => renameActivePlate(event.target.value)} /><button aria-label="Add plate" title="Add plate" onClick={() => addPlate()}>＋</button><button aria-label={activePlate?.locked ? "Unlock plate" : "Lock plate"} title={activePlate?.locked ? "Unlock plate" : "Lock plate"} onClick={togglePlateLock}>{activePlate?.locked ? "🔓" : "🔒"}</button><button aria-label="Duplicate plate" title="Duplicate plate" onClick={duplicateActivePlate}>⧉</button><button aria-label="Clear plate" title="Clear plate" onClick={clearActivePlate}>⌫</button><button aria-label="Delete plate" title="Delete plate" disabled>×</button><button aria-label="Unlock all plates and parts" title="Unlock all plates and parts" onClick={unlockAll}>↺</button></aside>}</div>
        {plates.length > 1 && <section className="multi-plate-canvas" aria-label="Side-by-side plate workspace">{plates.map((plate, index) => { const platePlacements = allPlacements.filter((placement) => placement.plateId === plate.id), isActive = plate.id === activePlateId; return <article key={plate.id} className={`multi-plate-card ${isActive ? "active" : ""}`} onClick={() => { if (!isActive) { setActivePlateId(plate.id); setSelectedPlacementId(null); setSelectedPlacementIds([]); setLayoutOptions([]); } }} onDragOver={(event) => { if (!plate.locked) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); dropOnPlate(plate.id); }}><header><span>{String(index + 1).padStart(2, "0")}</span><strong>{plate.locked ? "Locked · " : ""}{plate.name}</strong><small>{platePlacements.length} parts</small></header><div className="multi-plate-frame" style={{ aspectRatio: `${bedWidth}/${bedDepth}` }}><svg className="build-plate" viewBox={`0 0 ${bedWidth} ${bedDepth}`} aria-label={`${plate.name} build plate`}><defs><pattern id={`minorGrid-${plate.id}`} width={GRID} height={GRID} patternUnits="userSpaceOnUse"><path d={`M ${GRID} 0 L 0 0 0 ${GRID}`} fill="none" stroke="#2e2e38" strokeWidth="0.45" /></pattern><pattern id={`majorGrid-${plate.id}`} width={GRID * 5} height={GRID * 5} patternUnits="userSpaceOnUse"><rect width={GRID * 5} height={GRID * 5} fill={`url(#minorGrid-${plate.id})`} /><path d={`M ${GRID * 5} 0 L 0 0 0 ${GRID * 5}`} fill="none" stroke="#3a3a47" strokeWidth="0.75" /></pattern></defs><rect width={bedWidth} height={bedDepth} fill={`url(#majorGrid-${plate.id})`} /><rect className="edge-safety" x={EDGE_MARGIN} y={EDGE_MARGIN} width={Math.max(0, bedWidth - EDGE_MARGIN * 2)} height={Math.max(0, bedDepth - EDGE_MARGIN * 2)} />{platePlacements.map((placement) => { const part = parts.find((item) => item.id === placement.partId); if (!part) return null; const footprint = renderedFootprint(placement, part), b = bounds(footprint), selected = isActive && (selectedPlacementIds.includes(placement.id) || selectedPlacementId === placement.id); return <g key={placement.id} className={`plate-part ${placement.nested ? "is-nested" : "is-staged"} ${selected ? "is-selected" : ""}`}><path d={footprintPath(footprint)} fillRule="evenodd" fill={placement.colliding ? "#ff5f5f" : part.color} fillOpacity={placement.nested ? "0.86" : "0.62"} stroke={selected ? "#9b8dff" : placement.colliding ? "#ff5f5f" : "#555563"} vectorEffect="non-scaling-stroke" /><text x={(b.minX + b.maxX) / 2} y={(b.minY + b.maxY) / 2} textAnchor="middle" dominantBaseline="middle" fontSize="8">{placement.copy}</text></g>; })}{!platePlacements.length && <text x={bedWidth / 2} y={bedDepth / 2} textAnchor="middle" className="empty-plate-text">Empty plate</text>}</svg></div>{isActive && <aside className="plate-control-rail" aria-label="Active plate controls" onClick={(event) => event.stopPropagation()}><input aria-label="Active plate name" value={plate.name} disabled={isWorking} onChange={(event) => renameActivePlate(event.target.value)} /><button aria-label="Add plate" title="Add plate" onClick={() => addPlate()}>＋</button><button aria-label={plate.locked ? "Unlock plate" : "Lock plate"} title={plate.locked ? "Unlock plate" : "Lock plate"} onClick={togglePlateLock}>{plate.locked ? "🔓" : "🔒"}</button><button aria-label="Duplicate plate" title="Duplicate plate" onClick={duplicateActivePlate}>⧉</button><button aria-label="Clear plate" title="Clear plate" onClick={clearActivePlate}>⌫</button><button aria-label="Delete plate" title="Delete plate" disabled={plates.length === 1} onClick={deleteActivePlate}>×</button><button aria-label="Unlock all plates and parts" title="Unlock all plates and parts" onClick={unlockAll}>↺</button></aside>}</article>; })}</section>}
        <div className={`status-strip ${collisionCount ? "has-error" : ""}`}><span>{message}</span><span>{collisionCount ? `${collisionCount} collision${collisionCount === 1 ? "" : "s"}` : placements.length ? "No collisions" : "Waiting for parts"}</span></div>
      </section>

      <aside className="sidebar controls-sidebar">
        <div className="panel-heading"><div><span className="eyebrow">WORKFLOW</span><h2>Plan a build plate</h2></div></div>
        <section className="workflow-steps" aria-label="PrintNest workflow"><div className={!parts.length ? "active" : "complete"}><b>1</b><span><strong>Import parts</strong><small>Drop STEP or STL files in the Parts panel.</small></span></div><div className={parts.length && !allPlacements.length ? "active" : allPlacements.length ? "complete" : ""}><b>2</b><span><strong>Set up & nest</strong><small>Choose the printer, clearance, then create a layout.</small></span></div><div className={allPlacements.length ? "active" : ""}><b>3</b><span><strong>Review & export</strong><small>Check the result, then download the ready-to-slice 3MF.</small></span></div></section>
        <div className="workflow-phase"><span>1</span><div><strong>Set up</strong><small>Choose the printer and safe spacing.</small></div></div>
        ...
        <div className="control-group clearance-control" onWheel={(event) => { event.preventDefault(); event.stopPropagation(); adjustClearanceByWheel(event.deltaY); }}><div className="label-with-help"><label htmlFor="clearance">Part clearance</label><HelpTip label="part clearance">Minimum edge-to-edge gap between models. Scroll or drag for quick adjustment; collision checks run after you pause. This is separate from the fixed 2 mm plate-edge safety.</HelpTip></div><div className="range-row"><input id="clearance" type="range" min="0" max="15" step="0.1" value={clearance} style={{ background: `linear-gradient(to right, var(--accent) ${clearance / 15 * 100}%, #3b3b48 ${clearance / 15 * 100}%)` }} onChange={(e) => queueClearanceUpdate(Number(e.target.value))} /><label className="clearance-number"><input aria-label="Part clearance in millimetres" type="number" min="0" max="15" step="0.1" inputMode="decimal" value={clearanceDraft} onChange={(e) => setClearanceDraft(e.target.value)} onBlur={() => { if (!clearanceDraft.trim()) { setClearanceDraft(clearance.toFixed(1)); return; } const next = Number(clearanceDraft); if (Number.isFinite(next)) updateClearance(next); else setClearanceDraft(clearance.toFixed(1)); }} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} /><span>mm</span></label></div><div className="slider-labels"><span>0</span><span>15 mm</span></div></div>
        <div className="workflow-phase nest-phase"><span>2</span><div><strong>Nest layout</strong><small>Use the solver to rotate and arrange your parts.</small></div></div>
        <div className="primary-nest"><strong>{nestActionLabel}</strong><span>{nestScopeNote}</span><select aria-label="Nesting scope" value={primaryNestAction} onChange={(event) => setPrimaryNestAction(event.target.value as typeof primaryNestAction)}><option value="smart">Smart scope — use active plate first</option><option value="current">Active plate only</option><option value="all">Use existing plates</option><option value="add">Add plates for overflow</option></select><button className="button primary nest-button" disabled={!parts.length || isImporting || isWorking || (primaryNestAction === "current" && activePlate?.locked)} onClick={runPrimaryNest}><span>{nestActionLabel}</span><b>→</b></button></div>
        <div className="control-group search-presets"><div className="label-with-help"><label>Solver quality</label><HelpTip label="solver quality">Quick checks the main rotations. Balanced is the recommended everyday setting. Best fit searches more rotations and tighter interlocking options.</HelpTip></div><div>{(["quick", "balanced", "best"] as SearchPreset[]).map((preset) => <button key={preset} className={searchPreset === preset ? "active" : ""} onClick={() => applySearchPreset(preset)}>{preset === "best" ? "Best fit" : preset[0].toUpperCase() + preset.slice(1)}</button>)}</div><p className="field-note">{searchQualityNote} Expected time: {estimateLabel}.</p></div>
        <div className="mode-switch" role="group" aria-label="Nesting detail"><button className={uiMode === "simple" ? "active" : ""} onClick={() => setUiMode("simple")}>Essentials</button><button className={uiMode === "advanced" ? "active" : ""} onClick={() => setUiMode("advanced")}>Advanced nesting</button></div>
        ...
        {uiMode === "advanced" && <div className="control-group repeat-pattern-options"><div className="label-with-help"><label htmlFor="fill-mode">Fill bed options</label><HelpTip label="fill bed options">Fill bed uses the same rotation-aware solver as Nest. The default keeps existing placements; choose repack only when the active plate may be rearranged.</HelpTip></div><select id="fill-mode" className="printer-select" value={fillMode} onChange={(event) => setFillMode(event.target.value as typeof fillMode)}><option value="remaining">Keep existing positions</option><option value="repack">Repack active plate for maximum instances</option><option value="existing">Fill across existing plates</option><option value="batch">Fill for the complete-set target</option></select></div>}
        <button className="button orient-button" disabled={isWorking || !parts.some((p) => p.meshes.length)} onClick={() => openOrientation((selectedPartId && parts.find((part) => part.id === selectedPartId)?.meshes.length ? selectedPartId : parts.find((p) => p.meshes.length)?.id) ?? "")}>Review imported orientation</button><p className="orientation-note">Optional: imported pose is kept until you explicitly place a face down.</p>
        <div className="workflow-phase review-phase"><span>3</span><div><strong>Review & export</strong><small>Check the placement result, resolve any remaining parts, then export.</small></div></div>
        ...
        <section className="slicer-handoff" aria-label="Export 3MF"><div className="label-with-help"><div><strong>Export 3MF</strong><span>{plates.length > 1 ? "One multi-plate project 3MF for Bambu Studio and OrcaSlicer." : "A portable Core 3MF for the active plate."}</span></div><HelpTip label="3MF export">Single-plate exports use the public 3MF Core specification. Multi-plate projects add the Bambu Studio / OrcaSlicer plate map so every plate remains separate in one file. No slicer presets, account data, or uploads are included.</HelpTip></div><button className="button primary slicer-download" disabled={!allPlacements.length || isWorking} onClick={download3mf}>Export 3MF</button><p>{plates.length > 1 ? "Open in Bambu Studio or OrcaSlicer, then select and slice each planned plate." : "Open the exported 3MF in any slicer that supports Core 3MF."}</p></section>
        ...
        {unplacedCount > 0 && <section className="unplaced-help" aria-label="Unassigned part actions"><strong>{unplacedCount} requested instance{unplacedCount === 1 ? "" : "s"} still need a place</strong><p>Review the reason below, then choose the recovery action that matches it.</p><ul>{unplacedByPart.map(({ part, count }) => { const readiness = partReadiness(part); return <li key={part.id}><span><b>{part.name}</b><small>{readiness.tone === "blocked" ? readiness.text : "No legal position found with the current clearance and layout."}</small></span><b>{count}</b></li>; })}</ul><div><button onClick={() => { setShowUnplaced((value) => !value); setSelectedPlacementId(null); setSelectedPartId(unplacedByPart[0]?.part.id ?? null); }}>{showUnplaced ? "Clear highlight" : "Highlight affected"}</button><button onClick={() => distributeAcrossPlates(parts, false, false)}>Use existing plates</button><button onClick={() => distributeAcrossPlates(parts, true, false)}>Add plates for overflow</button><button onClick={reduceQuantitiesToFitted}>Use fitted quantities</button><button onClick={tryHigherEffort} disabled={searchPreset === "best"}>Try a deeper search</button></div></section>}
    ...
    {orientingPart && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="orientation-title"><section className="orientation-modal">
      ...
      <div className="orientation-body"><div className="viewer-panel"><Suspense fallback={<div className="viewer-loading">Loading 3D preview…</div>}><OrientationViewer meshes={orientingPart.meshes} orientation={orientingPart.orientation} onFaceSelected={layFaceDown} /></Suspense><div className="viewer-plate-label">BUILD PLATE · TOP SURFACE</div><div className="viewer-hint"><span>Click</span> a face to place it down immediately · drag to orbit · right-drag to pan · scroll to zoom</div></div>
        <aside className="orientation-controls"><span className="eyebrow">ORIENTATION WORKFLOW</span><h3>Click the face that should touch the plate</h3><p>The dark plane is the top of the build plate. Clicking a face immediately rests it on that plane; orbit and pan first until you can clearly see the face you want. This preview changes nesting only—your original model stays intact for 3MF export.</p>
    ...
    {workingLabel && <div className={`working-overlay ${nestingMode ? "is-nesting" : ""}`} role="status" aria-live="assertive" aria-label={workingLabel}><div className="working-card"><div className="working-spinner" aria-hidden="true"><span /><span /><span /></div><strong>{workingLabel}</strong>{nestingMode ? <><p>{nestProgress.placed} placed · {nestProgress.processed} of {nestProgress.total} requested parts · {nestProgress.candidateChecks.toLocaleString()} positions tested · {nestProgress.attempts} search pass{nestProgress.attempts === 1 ? "" : "es"}</p><div className="working-progress determinate" aria-hidden="true"><span style={{ width: `${nestProgress.total ? nestProgress.processed / nestProgress.total * 100 : 0}%` }} /></div><button type="button" className="working-stop" disabled={stopRequested} onClick={stopNesting}>{stopRequested ? "Stopping…" : nestingMode === "fill" ? "Stop filling" : "Stop nesting"}</button></> : <><p>Complex models and dense plates can take a moment.</p><div className="working-progress" aria-hidden="true"><span /></div>{cancellableTask && <button type="button" className="working-stop" disabled={stopRequested} onClick={cancelCurrentTask}>{stopRequested ? "Cancelling…" : cancellableTask === "orientation" ? "Cancel orientation" : "Stop adding instances"}</button>}</>}</div></div>}
  </main>;
}
