...
export default function Home() {
  ...
  return <main className="app-shell" aria-busy={isWorking}>
    <header className="topbar"><div className="brand"><img className="brand-logo" src="/logo.png" alt="BumpMesh" width="24" height="24" /><span>BumpMesh <small>by <a href="https://www.youtube.com/@CNCKitchen" target="_blank" rel="noreferrer">CNC Kitchen</a></small></span><span className="beta">BUILD PLATE TOOL</span></div><div className="privacy-note"><span className={`status-dot ${saveStatus === "error" ? "storage-error" : ""}`} /> {saveStatus === "saving" ? "Saving locally…" : saveStatus === "error" ? "Storage unavailable" : "Local-only"}</div><div className="topbar-actions"><button className="button secondary" onClick={undoLayout} disabled={!history.length || isWorking}>Undo</button><button className="button secondary" onClick={exportPlan} disabled={!allPlacements.length || isWorking}>Save layout</button><a className="button secondary github-link" href="https://github.com/Anvendt3D/BuildplateNester" target="_blank" rel="noreferrer" aria-label="GitHub repository">GitHub</a></div></header>
    <section className="workspace">
      ...
      <aside className="sidebar controls-sidebar">
        ...
        <details className="workflow-help"><summary>How to use PrintNest <span>＋</span></summary><ol><li>Import STEP or STL files. They stage on the active plate; overflow remains unassigned.</li><li>Orient models, select the printer, and manage plates from the tabs above the build area.</li><li>Lock completed plates or exact placements before further nesting.</li><li>Use <b>Nest current</b>, <b>Nest all</b>, production sets, or consolidation as needed.</li><li>Single-plate jobs export as portable Core 3MF. Multi-plate jobs export as one Bambu Studio / OrcaSlicer project 3MF.</li></ol><p>Everything stays local. Every operation enforces the fixed 2 mm bed-edge safety boundary.</p></details>
