...
export default function Home() {
  ...
  function updateClearance(value: number) {
    const safe = Math.max(0, Math.min(15, Number.isFinite(value) ? value : 0));
    setClearance(safe); setAllPlacements((current) => markAllCollisions(current, plates, bedWidth, bedDepth, safe));
  }
  ...
  return <main className="app-shell" aria-busy={isWorking}>
    ...
    <section className="workspace">
      ...
      <aside className="sidebar controls-sidebar">
        ...
        <details className="workflow-help"><summary>How to use PrintNest <span>＋</span></summary><ol><li>Import STEP or STL files. They stage on the active plate; overflow remains unassigned.</li><li>Orient models, select the printer, and manage plates from the tabs above the build area.</li><li>Lock completed plates or exact placements before further nesting.</li><li>Use <b>Nest current</b>, <b>Nest all</b>, production sets, or consolidation as needed.</li><li>Each plate is exported as a verified standard 3MF; multi-plate jobs are packaged as a ZIP.</li></ol><p>Everything stays local. Every operation enforces the fixed 2 mm bed-edge safety boundary.</p></details>
